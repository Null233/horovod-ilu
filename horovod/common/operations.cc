...
Response::ResponseType get_response_type(int response_type){
  Response::ResponseType Response_Type = Response::ResponseType::ALLGATHER;
  switch (response_type){
    case 0:
      Response_Type = Response::ResponseType::ALLGATHER;
      break;
    case 1:
      Response_Type = Response::ResponseType::ALLGATHER;
      break;
    case 2:
      Response_Type = Response::ResponseType::BROADCAST;
      break;
    case 3:
      Response_Type = Response::ResponseType::JOIN;
      break;
    case 4:
      Response_Type = Response::ResponseType::ADASUM;
      break;
    case 5:
      Response_Type = Response::ResponseType::ERROR;
      break;
  }
  return Response_Type;
}
int64_t get_size(int tensor_size){
  return static_cast<int64_t>(tensor_size);
}

void horovod_timeline_start_tensor(const char* tensor_name, int response_type, int tensor_size) {
  auto& timeline = horovod_global.timeline;
  std::string name = tensor_name;
  timeline.Start(name, get_response_type(response_type), get_size(tensor_size));
}

void horovod_timeline_end_tensor(const char* tensor_name) {
  auto& timeline = horovod_global.timeline;
  std::string name = tensor_name;
  timeline.End(name, nullptr);
}

void horovod_timeline_start_activity(const char* tensor_name, const char* activity) {
  auto& timeline = horovod_global.timeline;
  std::string name = tensor_name;
  timeline.ActivityStart(name, activity);
}

void horovod_timeline_end_activity(const char* tensor_name) {
  auto& timeline = horovod_global.timeline;
  std::string name = tensor_name;
  timeline.ActivityEnd(name);
}
...
bool horovod_TG_used() {
  //std::cout << "Call for horovod_TG_used()" << std::endl;
  return horovod_global.terngrad_state;
}

void horovod_enable_TG() {
  //std::cout << "Call for horovod_enable_TG()" << std::endl;
  horovod_global.terngrad_state = true;
}

void horovod_disable_TG() {
  //std::cout << "Call for horovod_disable_TG()" << std::endl;
  horovod_global.terngrad_state = false;
}
}
