...
class HorovodBasics(object):
    ...
    def timeline_start_tensor(self, tensor_name, response_type, tensor_size):
        self.MPI_LIB_CTYPES.horovod_timeline_start_tensor(
            ctypes.c_char_p(tensor_name.encode('utf-8')),
            ctypes.c_int(response_type),
            ctypes.c_int(tensor_size))

    def timeline_end_tensor(self, tensor_name):
        self.MPI_LIB_CTYPES.horovod_timeline_end_tensor(
            ctypes.c_char_p(tensor_name.encode('utf-8'))
        )

    def timeline_start_activity(self, tensor_name, activity):
        self.MPI_LIB_CTYPES.horovod_timeline_start_activity(
            ctypes.c_char_p(tensor_name.encode('utf-8'),),
            ctypes.c_char_p(activity.encode('utf-8'))
        )
    
    def timeline_end_activity(self, tensor_name):
        self.MPI_LIB_CTYPES.horovod_timeline_end_activity(
            ctypes.c_char_p(tensor_name.encode('utf-8'))
        )
    ...
    def terngrad_used(self):
        """Returns True if TernGrad compressor is used during runtime.

        Returns:
          A boolean value indicating whether TernGrad compressor is enabled.
        """
        TG_enabled = self.MPI_LIB_CTYPES.horovod_TG_used()
        return bool(TG_enabled)
        
    def enable_TG(self):
        """A function that enables TernGrad compressor."""
        self.MPI_LIB_CTYPES.horovod_enable_TG()

    def disable_TG(self):
        """A function that disables TernGrad compressor."""
        self.MPI_LIB_CTYPES.horovod_disable_TG()