...
terngrad_used = _basics.terngrad_used
enable_TG = _basics.enable_TG
disable_TG = _basics.disable_TG
...
def _allgather_async_func(tensor, name=None):
    return allgather_async(tensor, name)
def _allgather_sync_func(tensor, name):
    return allgather(tensor, name)
def _synchronize_func(handle):
...
def allreduce(tensor, average=None, name=None, compression=Compression.none, op=None,
    ...
    Returns:
        ...
        processes.
    """
    TG_flag = terngrad_used()
    if not compression == Compression.tern:
        disable_TG()
    tensor_compressed, ctx, shape = compression.compress(tensor, size(), _allgather_async_func, name)
    summed_tensor_compressed = HorovodAllreduce.apply(tensor_compressed, average, name, op,
                                                      prescale_factor, postscale_factor)
    output = compression.decompress(summed_tensor_compressed, ctx, size(), shape, torch.max(tensor), name, _synchronize_func)
    if TG_flag:
        enable_TG()
    return output
...
def grouped_allreduce(tensors, average=None, name=None, compression=Compression.none, op=None,
    ...
    if not compression == Compression.tern:
        disable_TG()
    tensors_compressed, ctxs, shapes = zip(*[compression.compress(t, size(), _allgather_async_func, name) for t in tensors])
    summed_tensors_compressed = HorovodGroupedAllreduce.apply(average, name, op,
                                                              prescale_factor, postscale_factor,
                                                              *tensors_compressed)
    output = [compression.decompress(t, ctx, size(), shape, 2.5, name, _synchronize_func) for t, ctx, shape in zip(summed_tensors_compressed, ctxs, shapes)]
    if TG_flag:
        enable_TG()
    return output
