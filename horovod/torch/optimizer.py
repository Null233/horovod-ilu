...
#from horovod.torch.mpi_ops import _broadcast_scaler_function
from horovod.torch.mpi_ops import _allgather_async_func, _allgather_sync_func
from horovod.torch.mpi_ops import _synchronize_func
...
from horovod.torch.mpi_ops import terngrad_used, enable_TG, disable_TG
...
class _DistributedOptimizer(torch.optim.Optimizer):
    ...
    def _allreduce_grad_async(self, p):
        ...
        tensor_compressed, ctx, shape = self._compression.compress(tensor, size(), _allgather_sync_func, name)
        ...
        if self.op == Average:
            ...
            if terngrad_used():
                ...
                postscale_factor = size() * self.gradient_predivide_factor
            else:
                prescale_factor = 1.0 / self.gradient_predivide_factor
    ...
    def _grouped_allreduce_grad_async(self, ps):
        ...
        tensors_compressed, ctxs, shapes = zip(*[self._compression.compress(p.grad, size(), _allgather_sync_func, name) for p in ps])
        ...
        for p, (handle, ctx) in self._handles.items():
            ...
            if isinstance(p, tuple):
                ...
                for gp, output, gctx in zip(p, outputs, ctx):
                    ...
                    gp.grad.set_(self._compression.decompress(output, gctx, size(), gp.grad.shape, torch.max(gp.grad),self._parameter_names.get(p),_synchronize_func))
                ...
                if self._groups is not None:
                    ...
                    if self._group_counts[group] != 0:
                        self._group_counts[group] = 0
                if p.grad.is_sparse:
                    aggregated = self._compression.decompress(output, ctx, size(), p.grad.shape, torch.max(p.grad), self._parameter_names.get(p),_synchronize_func)
                    ...
                    p.grad.set_(self._compression.decompress(output, ctx, size(), p.grad.shape, 0.1, self._parameter_names.get(p), _synchronize_func))
        self._handles.clear()
...
class _DistributedAdasumOptimizer(torch.optim.Optimizer):
    ...
    def _allreduce_grad_async(self, p):
        ...
        # allreduce as before
        tensor_compressed, ctx, shape = self._compression.compress(p, size(), _allgather_sync_func, name)
    ...
    def step(self, closure=None):
        ...
        for p, (handle, ctx) in self._handles.items():
            ...
            #print("--------optimizer--_DistributedAdasumOptimizer--step()--p.grad:")
            #print(p.grad.shape)

            delta = self._compression.decompress(delta, ctx, size(), p.grad.shape, torch.max(p.grad), self._parameter_names.get(p),_synchronize_func)
            start = self._starting_models[p]
...
def DistributedOptimizer(optimizer, named_parameters=None,
    ...
    if compression == Compression.tern:
        enable_TG()
    else:
        disable_TG()
